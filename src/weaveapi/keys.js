...
function decodeX962PrivateKey(privateKey) {
    const decoded = ASN1.decode(Buffer(privateKey, "base64"));
    const pk = decoded.sub[2].sub[0].sub[1];
    return pk.stream.enc.slice(pk.stream.pos + 2, pk.stream.pos + 2 + pk.length);
}
